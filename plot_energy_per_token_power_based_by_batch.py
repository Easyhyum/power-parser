...
def plot_energy_per_token_power_based_by_batch(metrics: pd.DataFrame, output_dir: Path) -> None:
    ...
    for bs in batch_sizes:
        ...
        for model_name in model_names:
            ...
            line = (
                sub.groupby("sm_clock", as_index=False)["energy_per_token_power_based"]
                .mean()
        ...
        plt.title(f"Energy per token (power-based) by model (batch_size={int(bs)})")
        plt.grid(True, alpha=0.3)
        plt.legend(title="model_name")
        plt.tight_layout()

        file_name = f"energy_per_token_power_based_bs{int(bs)}.png"
...
def summarize_decrease_as_batch_increases(
    metrics: pd.DataFrame, output_dir: Path
) -> pd.DataFrame:
    """
    model_name, sm_clock 단위로 batch_size 증가에 따른
    energy_per_token_power_based 감소량/감소율을 계산한다.
    """
    output_dir.mkdir(parents=True, exist_ok=True)

    grouped = (
        metrics.groupby(["model_name", "sm_clock", "batch_size"], as_index=False)[
    ...
    grouped["prev_batch_size"] = grouped.groupby(["model_name", "sm_clock"])[
        "batch_size"
    ].shift(1)
    grouped["prev_energy_per_token"] = grouped.groupby(["model_name", "sm_clock"])[
        "energy_per_token_power_based"
    ].shift(1)

    grouped["abs_decrease"] = (
        grouped["prev_energy_per_token"] - grouped["energy_per_token_power_based"]
    )
    grouped["pct_decrease"] = (
        grouped["abs_decrease"] / grouped["prev_energy_per_token"] * 100.0
    )

    summary = grouped.dropna(subset=["prev_batch_size", "prev_energy_per_token"]).copy()
    summary = summary.rename(
        columns={
            "energy_per_token_power_based": "curr_energy_per_token",
            "batch_size": "curr_batch_size",
        }
    )

    summary_path = output_dir / "energy_per_token_decrease_by_batch_step.csv"
    summary.to_csv(summary_path, index=False)
    print(f"저장 완료: {summary_path}")

    # 모델별로 감소율(직전 batch 대비 %) 플롯 생성
    for model_name in sorted(summary["model_name"].dropna().unique()):
        ...
        for sm_clock in sorted(sub_model["sm_clock"].dropna().unique()):
            sub_clock = (
                sub_model[sub_model["sm_clock"] == sm_clock]
                .sort_values("curr_batch_size")
            )
            if sub_clock.empty:
                continue
        ...
        plt.ylabel("Decrease vs previous batch (%)")
        plt.title(f"Batch increase effect by SM clock ({model_name})")
        plt.grid(True, alpha=0.3)
        plt.legend(title="sm_clock")
        plt.tight_layout()

        model_safe = sanitize_name(str(model_name))
        out_path = output_dir / f"decrease_pct_by_batch_{model_safe}.png"
        plt.savefig(out_path)
        plt.close()
        print(f"저장 완료: {out_path}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "모든 CSV를 합쳐서 batch_size별 에너지 라인차트(모든 model_name 범례 포함)를 생성합니다."
        )
    )
    parser.add_argument(
        "log_dir",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="출력 디렉터리 (기본값: <log_dir>/analysis_output)",
    )
    ...
    output_dir = Path(args.output_dir) if args.output_dir else (log_dir / "analysis_output")

    df = load_data(log_dir)
    metrics = compute_metrics(df)
    plot_energy_per_token_power_based_by_batch(metrics, output_dir)
    summarize_decrease_as_batch_increases(metrics, output_dir)
